...
public class Scope3_1 {
    public static void main(String[] args) {
        /* 변수를 선언한 시점부터 변수를 사용할 수 있게 해도 되지 않을까..?
        * 왜 복잡하게 접근 범위라는 개념을 만든것인가?*/
        ...
        * 그런데 이 코드는 좋은 코드라고 보기에는 어렵다.
        * 임시 변수 temp는 if 조건이 만족할 때 임시로 잠깐 사용하는 변수이다.
        * 그런데 임시 변수 temp main() 코드 블록에 선언되어 있다.
        * 이렇게 되면 다음과 같은 문제가 발생함 */

        /* 비효율적인 메모리 사용: temp 는 if 코드  블록에서만 필요하지만,
        main() 코드 블록이 종료될 때 까지 메모리에 유지된다.
        따라서 불필요한 메모리가 낭비된다.
        만약 if 코드 블록 안에 temp 를 선언했다면 자바를 구현하는 곳에서,
        if 코드 블록의 종료 시점에 이 변수를 메모리에서 제거해서 더 효율적으로 메모리를 사용할 수 있다.
